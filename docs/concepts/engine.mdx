...
### Docker environment paths and variables

`DockerEnv(container_env={...})` passes an explicit environment mapping when it
creates a container. Relative filesystem paths resolve under the configured
container workdir; absolute paths are interpreted as absolute paths inside the
container. QitOS does not copy benchmark- or task-specific host environment
...
Handled exceptions — anything the recovery layer catches during DECIDE/ACT, plus fatal `init_state` and env-setup failures — are reported through `Engine._report_runtime_exception()`: one stderr line with the full traceback, a `RECOVER` trace event carrying the structured payload, and (when `QITOS_ERROR_LOG` or `QITOS_TRACE_DIR` is set) an appended `step_error.log`. The last payload is exposed as `last_error` in the `EngineResult` run summary, so a run that ends with `unrecoverable_error` keeps usable diagnostics even under redirected output.

Tool-call loop detection can be disabled for long-running agents that intentionally repeat calls: `ContextConfig(tool_call_loop_detection_enabled=False)` (default `True`, with `loop_max_repeats` controlling the threshold).
