...
Install the pinned static-analysis toolchain with Python 3.12.7:
...
Run the full-package no-regression ratchet:

```bash
python scripts/static_quality.py check
```

The stable commands must remain at zero errors. The ratchet covers all active
`qitos` packages and compares existing findings with the committed baseline.
It fails on new findings and stale baseline entries. See
`quality/README.md` before shrinking the baseline or requesting an expiring
maintainer exception.

CI keeps the stable lint/type surface separate from the full-package ratchet.
Repository-intent roles for required-candidate, advisory, stale, and release
jobs are listed in `docs/internal/ci-job-ownership.md`; that inventory does not
claim to represent external GitHub branch-protection settings.
