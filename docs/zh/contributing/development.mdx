...
使用 Python 3.12.7 安装固定版本的静态分析工具链：
...
运行全包 no-regression ratchet：

```bash
python scripts/static_quality.py check
```

稳定层命令必须保持零错误。ratchet 覆盖所有 active `qitos` package，并将
现有 finding 与提交态 baseline 比较；出现新 finding 或本应删除的陈旧
baseline 条目时都会失败。缩小 baseline 或申请带到期日的 maintainer
exception 前，请阅读 `quality/README.md`。

CI 会将稳定层 lint/type 与全包 ratchet 保持为独立门禁。
`docs/internal/ci-job-ownership.md` 记录了 required-candidate、advisory、
stale 和 release job 的仓库内预期角色；该清单不声称代表外部 GitHub
