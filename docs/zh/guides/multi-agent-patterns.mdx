...
每个独立 child 显式分配两次请求预算。省略预算可能让第一个 child 预留 parent 的全部剩余请求，使后续 worker 获得零预算。完成不代表可以隐式退款或重放未知 effect。
...
[Source file](https://github.com/WhitzardAgent/WhitzardOS/blob/f7d4b2d666a156d361da496a41868278f84ffabf/examples/tutorials/notes/handoff.py) (可选；全部所需代码已在本页)
