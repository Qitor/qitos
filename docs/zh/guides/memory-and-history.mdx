...
先阅读 `run` 中的 factory 绑定。现有 YAML loader 接受 memory 与 compaction 名称；现有 Python `dataclasses.replace` 配置提供 `context.budget_policy`，并显式设置 `context.allow_codec_loss=True`。YAML loader 不接受这两个 context service 字段。这是示例明确展示的损失 opt-in，不是 adapter 或 compactor 内部自动启用的设置。
