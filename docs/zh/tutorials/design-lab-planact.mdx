...
完整实现路径: `examples/projects/planact_research/src/qitos_lab_planact/agent.py`.

## 从完整源码提取：设计增量

下列片段由完整项目同步生成，不是另一份手工维护的实现。完整安装文件在后面。

{/* tutorial-snippet:src/qitos_lab_planact/agent.py:design */}
```python
...
```python title="src/qitos_lab_planact/__main__.py"
...
```json title="src/qitos_lab_planact/tasks.json"
[
  {
...
```python title="src/qitos_lab_planact/evaluate.py"
...
```python title="src/qitos_lab_planact/agent.py"
...
```python title="src/qitos_lab_planact/__init__.py"
...
```python title="src/qitos_lab_planact/with_notebook.py"
