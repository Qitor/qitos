...
Chat streaming 将支持的 provider reasoning 与可见回答分开保留。重放需要声明 capability 且 provider/API scope 匹配，否则必须显式处理 codec loss。Owned cleanup 失败保留原 typed 请求错误及安全的 usage/partial 事实；仅清理失败也不算成功。离线组合消费者通过内置 adapter 验证这些合同，不代表真实 provider 已通过 live 资格。
