...
Read the factory bindings in `run`. The existing YAML loader accepts the memory and compaction names. The existing Python `dataclasses.replace` configuration supplies `context.budget_policy` and explicitly sets `context.allow_codec_loss=True`; those context service keys are not accepted by the YAML loader. This is a visible loss opt-in for the example, never a setting enabled inside the adapter or compactor.
