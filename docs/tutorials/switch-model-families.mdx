...
Chat streaming retains supported provider reasoning separately from the visible answer. Replaying provider reasoning requires declared capability and matching provider/API scope; otherwise codec loss is explicit. Owned cleanup failures preserve the original typed request failure and safe usage/partial facts; a cleanup-only failure is not success. The offline combined consumer exercises these contracts through the built-in adapter, without claiming live provider qualification.
...
```python
from qitos.models.harness_adapter import build_model_for_preset
