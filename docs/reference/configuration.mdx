...
The same strict `AgentConfig` and composition root are available from Python:
...
The canonical `qitos.agent/v1` YAML root has exactly `schema`, `agent`,
...
The default local resolver reads a private `0600` credential file outside the
repository. `FakeCredentialResolver` supports deterministic tests.
...
the reference and source digest, but never the resolved secret.
