...
R1 context configuration supports the following strict YAML. `budget` resolves through the existing explicit extensions factory boundary. No callable, client or credential value is persisted. Without loss opt-in, ordinary lossless requests still run; requests requiring loss fail before provider dispatch.
...
```bash
python -m pip install "qitos[openai] @ git+https://github.com/WhitzardAgent/WhitzardOS.git@f7d4b2d666a156d361da496a41868278f84ffabf"
```
...
[Source file](https://github.com/WhitzardAgent/WhitzardOS/blob/f7d4b2d666a156d361da496a41868278f84ffabf/examples/tutorials/notes/real_notes.py)
