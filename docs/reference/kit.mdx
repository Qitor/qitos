...
```python
from qitos.kit.tool.web.http import HTTPGet, HTTPPost
...
```python
from qitos.recipes.benchmarks.cybench import SubmitAnswer
```
...
| `default_headers` | `None` | Explicit HTTP headers applied to every client request; values are caller-owned and are not inferred from task-specific environment variables |
