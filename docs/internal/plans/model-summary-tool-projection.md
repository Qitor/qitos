...
- Preserve the canonical `ToolResult.output` unchanged for reduction and
  trace persistence.
- Budget trimming (`_tool_output_for_budget`) truncates the projected
  summary, not the raw structured dict.
- Per-tool redaction behavior applies only when a tool does not supply a
  model summary (tools own their privacy boundary via the summary they
  choose to expose).
...
3. A tool without a summary preserves existing behavior.
