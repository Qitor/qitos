...
## Session-first project path

For a new durable, sandboxed agent project, start with:

```bash
qit new --agent-name my_agent --output-dir . --no-input
python -m pip install ./my_agent
python -m pytest my_agent/tests
```

The generated test uses a deterministic provider in its temporary test workspace;
configured execution uses the Docker sandbox and explicit credential resolution.
See [G5 migration](/reference/g5-migration) for restore, immutable fork, retained
artifacts, explicit publication and journal inspection. The older minimal demo
below operates on its seeded host workspace; it is not the isolation reference.
...
<Steps>
  ...
  <Step title="Run the minimal coding agent">
    ...
    trace_run: .../runs/trajectory.journal
