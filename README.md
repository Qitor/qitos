...
- **Evidence-backed v4 integration ledger**: [`docs/progress.md`](docs/progress.md) now records exact lane heads, integration dispositions, code-review findings, cross-lane contract blockers, merge order, and G1/G2 checklists. First-wave branches are tracked separately from what is actually present and qualified on the integration branch.
- **Engineering-quality audit and four-lane execution track**: the [evidence-backed audit](docs/engineering-quality-audit.md) covers whole-package gates, failure/durability semantics, resource lifecycle, duplicated abstractions, optional dependencies, and test trust. The [four-lane playbook](docs/v4/11-four-lane-execution-playbook.md) turns Tasks 02–05 and 08–10 into explicit Quality, Conversation/Context, Tool/Runtime, and Trajectory/Convergence production lines with file leases, handoff fixtures, merge waves, and evidence gates.
