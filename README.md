...
- **S2 closed; S3 dispatch contract frozen**: the durable single-agent vertical and clean-process restore were qualified, promoted, revalidated in the primary checkout, pushed, and cleaned up through `3af0ee3...`. The [S3 wave](docs/internal/plans/s3_durable_multi_agent_wave.md) now fixes A -> B -> C -> D producer ownership and the nineteen-item G4 process-loss gate. S3 implementation has not started: there is no Session fork runtime or durable multi-agent scheduler, Trajectory v2 remains unfrozen, and the candidate trajectory reader is not qita's default.
