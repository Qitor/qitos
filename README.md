...
- **G4-L3 R1 transport repair under requalification**: canonical immutable request options now cross one bounded JSON materialization boundary immediately before provider transport. DSV/GLM/Qwen nested thinking options project to fresh dict/list trees without mutating config, model defaults, RequestView, or Session state; invalid values fail with safe typed paths/categories. EngineResult, failed Session heads, Trajectory, and the qualification receipt retain the root failure code before pause. Focused Python 3.12.7 regression gates pass; a new live round remains unauthorized until the complete committed-byte offline matrix passes.
