...
- **G1 scalar-safe convergence accepted**: after the accepted R3 baseline was reopened by an independent C-P4 audit, G1-R4 now redacts forced-secret integer, float, boolean, null, and nested scalar leaves while preserving typed omitted counts, benign values, lossless canonical data, and conserved loss facts. ExchangeLog inherits the repair without a runtime change, D binds the exact C producer artifacts, and the Python 3.12.7 ratchet, lint/type, targeted, readiness, adversarial, and `1872 passed, 50 skipped` full-suite gates pass. S1 remains unimplemented and may start only from the final R4 baseline.
