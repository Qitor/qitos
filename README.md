...
- **G2-R2 contract repair qualified; promotion pending**: the candidate was replayed onto the audit-bearing integration baseline, then strict historical ToolResult parsing, capability types, diagnostic/ArtifactRef safety, semantic interface visibility, and distinct current/historical receipts were repaired. The repair tree passes 291 focused checks and `2104 passed, 50 skipped`; readiness is honestly 0/21 without receipts and 21/21 with exact receipts while runtime/Trajectory blockers remain. Primary-branch revalidation and clean worktree retirement still gate the sole S2 baseline.
