...
- **G4-L3 R1 transport repair crossed live dispatch**: canonical immutable request options now cross one bounded JSON materialization boundary immediately before provider transport. Fresh round `s3-g4-l3-1f3414e2567ef3f9` issued six real GLM requests and native coding tools, then exposed three deterministic qualification defects: the local step ceiling ended before final, the sandbox image lacked pytest, and a bounded large qita projection omitted metadata. The prompt and 12-request ceiling stay fixed; typed budget diagnostics, safe qita metadata, and an attested pytest-capable local image are being requalified before another live round.
