...
- **G2 stable-contract convergence complete**: the four S1 source candidates now converge on one typed identity vocabulary, extensible snapshot composition, canonical `ArtifactRef`, one current `ToolResult` writer with an isolated historical reader, provider-declared capabilities, and safe diagnostics. D independently qualifies all 17 S1 contract requirements against exact A/B/C producer commits and bytes. The beginner path gains no root export or Engine parameter; pause/restore/fork runtime and the future Trajectory writer/store/qita migration remain deliberately unimplemented and blocked.
