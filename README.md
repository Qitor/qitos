...
- **S3 deterministic candidate converged; release gate still blocked**: the fixed `851f790...` A -> B -> C -> D replay now provides Session fork, strict context/authority transfer, a reconstructable durable WorkGraph runtime, and read-only qita graph/timeline inspection. Twenty independent SQLite graph process-loss rounds plus twenty preparation-crash rounds pass without replaying completed or outcome-unknown work or creating a second fork child. No live-model matrix was supplied, so this remains an unpromoted candidate: Trajectory v2 is unfrozen/off, qita still defaults to frozen trace-v1, and no distributed or exactly-once claim is made. See the [G4 evidence](docs/internal/plans/s3_g4_convergence_evidence.md).
