...
- **G1 convergence candidate independently audited**: A → C → B → D is integrated and the Python 3.12.7 ratchet, lint/type, targeted, and full-suite gates pass. The audit nevertheless reopened G1 on C-P3: sensitive mapping keys and trace-safe omitted keys can bypass ToolResult redaction/loss accounting. S1 remains blocked pending that bounded repair and combined requalification.
...
- **G1 ToolResult boundary progress**: recursive JSON-only arguments fail before interceptors, permissions, and tool code, and nested canonical/legacy values are ownership-isolated. Scalar model/trace values use bounded redaction; complete mapping-key and omitted-key protection is the active C-P3 repair.
