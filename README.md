...
- **G4-L3 R1 transport repair crossed live dispatch**: immutable request options now cross one bounded JSON materialization boundary before provider transport. R1 round `s3-g4-l3-1f3414e2567ef3f9` exposed local step/image/qita defects; follow-up round `s3-g4-l3-d6fa110124a7c660` then recovered a malformed GLM response into native tools and final, but a pause policy tied to literal step zero missed the post-recovery boundary. The policy now pauses once per Session at its first successful boundary, while typed model failures suppress raw traceback/path output. A new live round remains gated on complete committed-byte offline qualification.
