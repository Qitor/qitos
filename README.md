...
- **G4-L3 qualification candidate**: a deterministic configured Docker workflow now proves native parallel read/grep, repository repair, pytest, safe pause, fresh-process restore, final output, real Trajectory/qita reads, and cleanup; the existing 40-round multi-agent process-loss suite remains authoritative. A new bounded live round is gated behind the full offline suite and will run GLM primary, DSV parity, then Qwen text plus typed native capability loss. The prior G4-L2 failure ledger remains immutable, and the candidate is not promoted or pushed unless the new round and every repository gate pass. See the [L3 evidence ledger](docs/internal/plans/s3_g4_l3_qualification_evidence.md).
