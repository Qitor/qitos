...
- **G4-L3 deterministic repairs qualified; live transport blocked at budget**: immutable request options cross one bounded JSON boundary; recovered Sessions pause safely; live fork IDs are parent-scoped; failures remain typed and non-echoing. Committed head `0f1435c...` passed `2415 passed, 50 skipped` plus all offline gates. Round `s3-g4-l3-739e066ca1e8172a` proved the GLM edit/test/pause/clean-process-restore/final/Trajectory/qita path, then its first multi-agent child ended with `provider_transport_failure` at the exact 12/12 profile ceiling. DSV/Qwen stayed at zero; promotion, push, and worktree cleanup remain blocked.
- **Stable declarative Agent launch**: canonical `schema: qitos.agent` files now drive the existing `AgentModule + Engine` path through `qit run --config agent.yaml` or `run_agent_config("agent.yaml")`. Loaded configuration is deeply immutable and produces a deterministic, secret-free digest carried through launch, Session restore, and Trajectory provenance. The old `qitos.agent/v1` spelling is reader-only compatibility. Protocol resolution now owns parser and codec selection, while configurable tool-use policy can require a real tool result before final output. See [Configuration](docs/reference/configuration.mdx).
...
- **G4-L3 offline-qualified; live primary blocked**: the configured Docker workflow and full repository gate pass, including native parallel read/grep, repair/test, safe pause, fresh-process restore, real Trajectory/qita reads, cleanup, and the authoritative 40-round multi-agent process-loss suite. Fresh live round `s3-g4-l3-71564b10447bf692` then stopped before its first provider request: immutable nested request options reached the codec as a `mappingproxy`, which was correctly rejected as non-JSON-compatible, but prevented the GLM Session from reaching pause. DSV and Qwen were not started. The candidate remains unpromoted/unpushed and every worktree is preserved. See the [L3 evidence ledger](docs/internal/plans/s3_g4_l3_qualification_evidence.md).
