...
- **G2 candidate delivered; independent promotion audit open**: the candidate converges typed session/work identities, snapshot ownership, ArtifactRef, RequestView, ToolResult evolution, provider capabilities, and exact receipts, and its full suite passes `2010 passed, 50 skipped`. Independent probes nevertheless reopened strict historical-schema parsing, capability types, diagnostic/privacy coverage, current-versus-historical receipts, interface visibility, promotion, and worktree retirement. One bounded G2-R2 closes those items before the four-lane S2 runtime wave may start.
