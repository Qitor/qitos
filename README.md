...
- **G1 final convergence accepted**: G1-R3 closes C-P3 with deterministic collision-safe sensitive-key placeholders, bounded trace-safe omitted data, and complete loss facts. ExchangeLog inherits the repaired public projections without a runtime change, D verifies the exact C producer artifacts, and the Python 3.12.7 ratchet, lint/type, targeted, readiness, adversarial, and full-suite gates pass. S1 contract packages are authorized only from the final accepted baseline.
...
- **G1 ToolResult boundary closed**: recursive JSON-only arguments fail before interceptors, permissions, and tool code; nested canonical/legacy values are ownership-isolated; and every model/trace-visible mapping key, including trace-safe omitted data, uses collision-safe redaction with explicit aggregate and per-field loss accounting.
