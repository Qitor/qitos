...
- **S3 deterministic candidate converged; live Agent workflow remains blocked**: the fixed `851f790...` A -> B -> C -> D replay provides Session fork, strict context/authority transfer, a reconstructable durable WorkGraph runtime, and read-only qita graph/timeline inspection. Twenty independent SQLite graph process-loss rounds plus twenty preparation-crash rounds pass without replaying completed or outcome-unknown work or creating a second fork child. G4-L2 real preflight proved native tools for DSV4/GLM and typed Qwen capability loss, but neither required coding/restore workflow completed and one profile exceeded its request cap. The candidate was not promoted or pushed. Trajectory remains unfrozen/off, qita still defaults to frozen trace compatibility, and no distributed or exactly-once claim is made. See the [G4 live evidence](docs/internal/plans/s3_g4_live_qualification_evidence.md).
