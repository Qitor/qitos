...
- **G2-R2 contract baseline promoted**: the candidate was replayed onto the audit-bearing integration baseline, then strict historical ToolResult parsing, capability types, diagnostic/ArtifactRef safety, semantic interface visibility, and distinct current/historical receipts were repaired. Baseline `c0f19cd...` passes 291 focused checks and `2104 passed, 50 skipped`; readiness is honestly 0/21 without receipts and 21/21 with exact receipts while runtime/Trajectory blockers remain. Primary revalidation passed and 17 superseded worktrees were cleanly retired without deleting their branch refs.
