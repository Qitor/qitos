...
- **Versioned conversation transaction contract**: the new module-level `qitos.core.conversation` contract preserves ordered multimodal exchanges, parallel tool-call/result correlation, queued steering, raw/parsed arguments, synthetic closure provenance, and opaque provider continuation without flattening reasoning. Strict `HistoryMessage` adapters and Lane C/D fixtures establish compatibility before any Engine or provider default is switched.
