...
- **Durable-session and native multi-agent v4 architecture**: [Task 12](docs/v4/12-session-runtime-and-persistence.md) defines one checkpoint-v2-backed session truth for safe pause, fresh-process resume, fork, and effect-aware recovery; [Task 13](docs/v4/13-durable-multi-agent-work-graph.md) defines distinct handoff, delegate, fan-out, spawn, fork, steer, and join semantics over durable work items. The [four-lane playbook](docs/v4/11-four-lane-execution-playbook.md) now makes quality a cross-lane gate after G1 and assigns future capacity to Session, Conversation/Context, Tools/Multi-Agent, and Trajectory/qita/DX.
...
- **Canonical tool outcomes and runtime ownership**: `ToolResult` now carries the lossless `qitos.tool_result/v1` action/tool outcome, while `ActionResult`, legacy dictionaries, and `model_summary` adapt at compatibility boundaries. A structural schema hard gate, lifecycle ownership matrix, deterministic durability-race proof, and versioned Lane B/D fixtures establish the Task 03A/09A contract without changing coding-tool, checkpoint, or MCP transport behavior.
...
- **G1 ToolResult boundary closure**: recursive JSON-only arguments fail before interceptors, permissions, and tool code; nested canonical/legacy values are ownership-isolated; and all model/trace-visible fields use one bounded, redacted projection with aggregate and per-field loss facts.
...
- **Crash-honest conversation transaction contract**: the module-level `qitos.core.conversation` contract now isolates nested mutable values, persists partial parallel-tool results immediately in completion order, resumes only missing slots after reload, and offers an explicit declaration-ordered query. Its continuation-redacted diagnostic view is not a privacy-safe export, and the in-repository fixtures are consumer simulations rather than independent Lane C/D adoption. Engine/provider defaults and canonical C1-R `ToolResult` alignment remain pending.
