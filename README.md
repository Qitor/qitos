...
- **G4-L3 transport and safe-pause repairs crossed live dispatch**: immutable request options now cross one bounded JSON boundary before provider transport. Rounds `s3-g4-l3-1f3414e2567ef3f9` and `s3-g4-l3-d6fa110124a7c660` exposed bounded local step/image/qita and post-recovery pause defects. Round `s3-g4-l3-08e956825bfb8f0f` then proved real GLM dispatch and safe pauses, but exposed a reused global fork idempotency key. Fan-out/join operation IDs are now parent-Session-scoped and enum-valued Session failures remain typed and non-echoing. A new live round remains gated on complete committed-byte offline qualification.
