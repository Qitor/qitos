...
- **Qualified static ratchet and auditable CI roles**: deterministic tests now prove every critical baseline transition, including new/stale debt, controlled growth, expiry, bootstrap, malformed diagnostics, and rules upgrades. Contribution checks no longer use GitHub's numeric `changed_files` value as a file array or mask pytest failures; required-candidate, advisory, stale, and release-only workflow roles are recorded without claiming access to branch-protection settings.
