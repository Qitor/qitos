...
def chunk(text='', reasoning=None, finish=None, calls=None, usage=None):
    return NS(choices=[NS(delta=NS(content=text, reasoning_content=reasoning,
...
def adapter(monkeypatch, items, *, asynchronous=False, response_error=False, client_error=False):
    ...
    return cls(model='offline', api_key='offline', base_url='https://offline.invalid/v1', max_tokens=10240), stream, client


@pytest.mark.parametrize('answer', ['', 'answer'])
@pytest.mark.parametrize('asynchronous', [False, True])
def test_reasoning_fragments_separate_once(monkeypatch, answer, asynchronous):
    model, stream, client = adapter(monkeypatch, [chunk(reasoning='first '),
        chunk(reasoning='second'), chunk(answer), chunk(finish='stop')], asynchronous=asynchronous)
    if asynchronous:
        async def consume():
...
def test_cleanup_priority(monkeypatch, asynchronous, mode, response_error, client_error):
    ...
    events += {'normal': [chunk(finish='stop')], 'eof': [],
               'transport': [ConnectionError(PRIVATE)],
               'contradiction': [chunk(finish='stop'), chunk('late')]}[mode]
    model, stream, client = adapter(monkeypatch, events, asynchronous=asynchronous,
                                    response_error=response_error, client_error=client_error)
    ...
    error = caught.value
    assert error.category == ('provider_exception' if mode == 'normal' else 'connection' if mode == 'transport' else 'stream')
...
def test_explicit_close_failure_is_typed(monkeypatch, asynchronous):
    model, stream, client = adapter(monkeypatch, [chunk('partial')], asynchronous=asynchronous,
                                    response_error=True, client_error=True)
...
def test_reasoning_tools_canonical_reread_and_capability_loss(monkeypatch):
    ...
    model, _, _ = adapter(monkeypatch, [chunk(reasoning='plan '), chunk(reasoning='one', calls=[call]),
    ...
    batch.record_result(ToolResultItem('result1', tx.assistant_item.exchange_id, call.identity,
                             call.batch_id, ToolResult(status='success', output='ok')))
    log = ExchangeLog.from_dict(log.to_persistence_dict())
    next_view = RequestView.from_exchange_log(log, target=target)
    reason = [part for item in next_view.selected_items if item['kind'] == 'assistant'
              for part in item['parts'] if part['kind'] == 'reasoning_block']
    ...
    supported = replace(capabilities, supports_reasoning_input=True, supports_reasoning_output=True,
...
def test_async_cancellation_retains_identity_and_cleans_both(monkeypatch):
    async def scenario():
        ...
        model, stream, client = adapter(monkeypatch, [], asynchronous=True,
                                        response_error=True, client_error=True)
