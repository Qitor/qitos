...
from qitos.core.session import AgentIdentity, RunIdentity, SessionIdentity, SessionLifecycle, WorkItemIdentity, AttemptIdentity
...
def descriptor(operation, session, work):
    return WorkDescriptor(operation, 'handoff', session.value, work.value,
                          [], [], [], {}, [], [], [], [], [], [], 0, 1)
...
def test_review_same_agent_different_work_and_generation(bound):
    ...
    for i, (work, session) in enumerate(zip(works, sessions)):
        ...
        graph.transfer_owner(work, expected_generation=0, to_agent_id=owner,
                             transfer_id=f'ownership:{i}', context_transfer_ref=f'context:{i}')
        graph.operation_receipts.append(WorkOperationReceipt(str(i), 'handoff', 'a'*64,
            'transfer_admitted', outcome_unknown=True, descriptor=replace(descriptor(str(i), session, work),
                transfer_receipts=[{'receipt_id': f'context:{i}'}] if bound else []).to_dict()))
...
@pytest.mark.parametrize('code', ['queue_capacity_exceeded', 'scheduler_unavailable', 'descriptor_resolution_failed'])
def test_known_non_dispatch_persists_and_retry_does_not_duplicate(code):
    ...
    with pytest.raises(WorkRuntimeError, match='operation_identity_conflict'):
        runtime.submit(graph=recovered, descriptor=replace(work, task_input={'changed': True}), persist=persist)
...
def test_dispatch_ack_loss_stays_unknown():
    ...
    saved = []
    with pytest.raises(WorkRuntimeError):
        runtime.submit(graph=graph, descriptor=work, persist=lambda: saved.append(graph.to_persistence_dict()))
...
def test_rejection_cannot_write_after_destination_claim(tmp_path):
    ...
    try:
        ...
        assert WorkGraph.from_canonical_dict(source.inspect().work_graph).operation_receipts[0].state == 'completed'
    finally:
        store.close()
    ...
    try:
        ...
        class PauseAgain(PauseOnce):
            def should_pause(self, context):
                return True
        fresh = RuntimeComposition(checkpoint_store=store, lifecycle_policy=PauseAgain(),
                                   work_runtime=DurableWorkRuntime(scheduler))
        fresh.bind_engine_resources(Engine(ForkAgent(), runtime=fresh))
