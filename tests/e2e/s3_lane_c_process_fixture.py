...
class Scheduler:
    ...
    def dispatch(self, request: WorkDispatch, worker) -> Handle:
        del worker
...
def create() -> None:
    ...
    runtime = RuntimeComposition(
        ...
        work_runtime=DurableWorkRuntime(scheduler, child_runner=lambda operation, payload: None),
    )
    session = Engine(LaneCAgent(), runtime=runtime).session("lane c process loss")
    completed = session.spawn("worker", task="completed", operation_id="spawn:completed")
    scheduler.handles[completed.worker_ref].finish()
    session.spawn("worker", task="missing", operation_id="spawn:missing")
    session.spawn("worker", task="unknown", operation_id="spawn:unknown")
