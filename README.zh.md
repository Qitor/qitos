...
- **G4-L3 R1 transport 修复正在重新资格验证**：canonical 不可变 request option 现在会在 provider transport 前穿过唯一一套有界递归 JSON materialization。DSV/GLM/Qwen 的嵌套 thinking 配置被投影为全新的 dict/list，不会污染 config、model default、RequestView 或 Session；非法值以安全的字段路径/类别 typed fail。EngineResult、failed Session head、Trajectory 与资格 receipt 会保留 pause 前的真实根因码。Python 3.12.7 focused regression 已通过；完整 committed-byte 离线矩阵通过前不会启动新 live round。
