...
- **G4-R5 框架一致性已通过**：代码头 `901e972...` 通过 2436 项测试，以及确定性 Session 隔离、持久请求预算恢复、有界 10 MB 投影、40 轮多智能体进程丢失恢复、Docker、隐私、打包与接口门禁。Informational GLM round `s3-g4-l3-a1146482fb3295ff` 发出两次精确计数的请求，未发现框架不变量失败；`malformed_structured_response` 属于模型能力证据，而不是框架发布阻断。当前等待纯 fast-forward promotion。
...
- **G4-L3 确定性修复已合格，live transport 在预算上限阻塞**：不可变 request option 穿过唯一的有界 JSON 边界，恢复后的 Session 能安全 pause，live fork ID 按 parent 隔离，failure 保持 typed 且不回显。提交 `0f1435c...` 通过 `2415 passed, 50 skipped` 与全部离线门禁。Round `s3-g4-l3-739e066ca1e8172a` 证明 GLM 的 edit/test/pause/clean-process-restore/final/Trajectory/qita 路径，但第一个多智能体 child 在精确 12/12 profile 上限处以 `provider_transport_failure` 结束。DSV/Qwen 保持零请求；promotion、push 与 worktree cleanup 继续阻塞。
- **稳定的声明式 Agent 启动**：canonical `schema: qitos.agent` 文件现在通过 `qit run --config agent.yaml` 或 `run_agent_config("agent.yaml")` 驱动现有 `AgentModule + Engine` 路径。加载后的配置深度不可变，并生成确定、无 secret 的 digest；该 digest 进入启动、Session restore 与 Trajectory provenance。旧 `qitos.agent/v1` 仅由兼容 reader 接受。protocol 统一决定 parser 与 codec，配置化 tool-use policy 还能要求真实工具结果后才允许 final。详见[配置文档](docs/zh/reference/configuration.mdx)。
...
- **G4-L3 离线资格通过，live primary 阻塞**：配置化 Docker 工作流与全量仓库门禁均通过，包括 native 并行 read/grep、修复/测试、safe pause、fresh-process restore、真实 Trajectory/qita、cleanup 及权威 40 轮多智能体进程丢失套件。全新 live round `s3-g4-l3-71564b10447bf692` 在第一次 provider request 前停止：不可变的嵌套 request option 以 `mappingproxy` 到达 codec，被正确拒绝为非 JSON-compatible，GLM Session 因而未到达 pause。DSV 与 Qwen 未启动。候选不提升、不推送，所有 worktree 保留。详见 [L3 证据账本](docs/internal/plans/s3_g4_l3_qualification_evidence.md)。
