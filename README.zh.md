...
- **G4-L3 R1 transport 修复已跨过真实 dispatch**：canonical 不可变 request option 现在会在 provider transport 前穿过唯一一套有界递归 JSON materialization。新 round `s3-g4-l3-1f3414e2567ef3f9` 发出了 6 次真实 GLM 请求并执行 native coding tools，随后暴露三项确定性资格缺陷：本地 step ceiling 在 final 前终止、sandbox 镜像缺少 pytest、大型 qita 有界投影遗漏 metadata。prompt 与每 profile 12 次请求上限保持不变；typed budget 诊断、安全 qita metadata 与含 pytest 的本机 attested 镜像必须完成重新资格后才会开新 live round。
