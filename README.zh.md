...
- **G4-L3 transport 与 safe-pause 修复已跨过真实 dispatch**：不可变 request option 现在会在 provider transport 前穿过唯一的有界 JSON 边界。Rounds `s3-g4-l3-1f3414e2567ef3f9` 与 `s3-g4-l3-d6fa110124a7c660` 暴露了本地 step/image/qita 与恢复后 pause 缺陷；round `s3-g4-l3-08e956825bfb8f0f` 随后证明真实 GLM dispatch 和安全 pause，但又暴露全局 fork 幂等键复用。现在 fan-out/join operation ID 按 parent Session 隔离，enum-valued Session failure 也保持 typed 且不回显。新 live round 仍须先通过 committed-byte 全量离线资格。
