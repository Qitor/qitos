...
- **G4-L3 R1 transport 修复已跨过真实 dispatch**：不可变 request option 现在会在 provider transport 前穿过唯一一套有界递归 JSON materialization。R1 round `s3-g4-l3-1f3414e2567ef3f9` 暴露本地 step/image/qita 缺陷；后续 round `s3-g4-l3-d6fa110124a7c660` 又将一次 malformed GLM 响应恢复为 native tools 与 final，但绑定 literal step zero 的 pause policy 漏掉了恢复后的成功边界。现在每个 Session 会在首次成功边界 pause，typed model failure 也不再输出 raw traceback/本地路径。新的 live round 仍必须先通过 committed-byte 全量离线资格。
