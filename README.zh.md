...
- **G2 候选已交付，独立 promotion 审计仍开放**：候选树已收敛 typed session/work identity、snapshot ownership、ArtifactRef、RequestView、ToolResult 演进、provider capability 与精确 receipts，并通过 `2010 passed, 50 skipped` 全量测试；但独立探针重新发现 historical schema strictness、capability 类型、诊断隐私覆盖、current/historical receipt、接口可见性、promotion 与 worktree 回收问题。必须先由单条 G2-R2 收口，之后才能启动四条 S2 运行时产线。
