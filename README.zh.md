...
- **G2-R2 契约修复已通过资格验证，promotion 尚待完成**：候选提交已重放到包含独立审计的集成基线，并修复 historical ToolResult 严格解析、capability 类型、诊断与 ArtifactRef 安全、接口预算语义及 current/historical receipts。修复树通过 291 项定向检查与 `2104 passed, 50 skipped` 全量测试；readiness 在无收据时为 0/21、精确收据时为 21/21，同时继续保留 runtime/Trajectory blocker。主分支复验与无强制 worktree 回收仍是唯一 S2 基线的前置门禁。
