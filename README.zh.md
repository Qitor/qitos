...
- **G4-L3 资格候选**：确定性的配置化 Docker 工作流已证明 native 并行 read/grep、修复、pytest、safe pause、fresh-process restore、final、真实 Trajectory/qita 读取及 cleanup；既有 40 轮多智能体进程丢失套件继续作为权威证据。全量离线门禁通过后才会启动全新 bounded live round，顺序为 GLM primary、DSV parity、Qwen text 加 typed native capability loss。旧 G4-L2 失败账本保持不可变；新轮和全部仓库门禁未通过前，不提升也不推送。详见 [L3 证据账本](docs/internal/plans/s3_g4_l3_qualification_evidence.md)。
