...
- **面向崩溃恢复的对话事务契约**：模块级 `qitos.core.conversation` 现在隔离嵌套可变值，按真实完成顺序立即持久化并行工具的部分结果，重载后只恢复缺失槽位，并提供显式的声明顺序派生查询。continuation-redacted diagnostic view 不是隐私安全导出，仓内 fixture 也只代表 consumer simulation，而非 Lane C/D 的独立采用；Engine/provider 默认路径与 C1-R canonical `ToolResult` 对齐仍待后续完成。
