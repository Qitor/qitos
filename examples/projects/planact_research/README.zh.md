...
分离规划与执行，并依据反馈更新计划。本课改编运行时思想，不复现论文的规划器训练或基准成绩。 [原始来源](https://arxiv.org/abs/2503.09572)。

## 框架与用户的分工

持久 phase 与 plan_version 区分角色；revise_plan 是统一工具调用，不是绕过记录的规划器 SDK 请求。static 对照关闭执行后的自动重规划。
...
练习：把每步重规划改成证据触发策略，同时比较失败和请求成本。

组合：组合笔记本的记忆适配器与独立验证扩展，不修改 Engine。此组合的安装验收仍待完成。
