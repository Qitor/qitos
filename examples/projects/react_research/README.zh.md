...
让动作与基于观察的决策交替推进。推理仅记录模型实际提供的内容，不编造隐藏思维。 [原始来源](https://react-lm.github.io/)。

## 框架与用户的分工

ResearchState 保留有限的近期观察；prepare 提供任务与证据，reduce 接收实际 ToolResult。每次请求和动作仍由 Engine 执行。
...
练习：替换证据排序策略，保持数值与来源检查器不变。

组合：与 PlanAct 运行同样任务，对比真实请求数、纠错行为和独立结果。
