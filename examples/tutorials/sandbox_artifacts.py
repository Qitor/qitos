...
Only --publish registers publication authority over answer.txt in a new fixture.
"""
import argparse
import hashlib
import json
...
from session_walkthrough import PauseAfterTool
...
class FakeProvider:
    ...
    def __init__(self, publish, stage=0):
        self.actions = [
            ("write_file", {"path": "answer.txt", "content": "42\n"}),
...
def run(root: Path, image: str, publish: bool):
    ...
    source = root / "source"
    source.mkdir()
    (source / "answer.txt").write_text("original\n", encoding="utf-8")
    config = AgentConfig(
    ...
    with build_agent_composition(config, model_override=FakeProvider(publish),
        ...
        session = composition.session("Write 42 and retain a large output")
        session.run()
        assert session.lifecycle.value == "paused"
        identity = session.session_id.value
    assert (source / "answer.txt").read_text() == "original\n"
    with build_agent_composition(config, model_override=FakeProvider(publish, stage=1),
        ...
        if publish:
            composition.tool_registry.register(SandboxPublicationTool(
                composition.env, paths=["answer.txt"],
        ...
        assert (source / "answer.txt").read_text() == ("42\n" if publish else "original\n")
