...
- Added the module-level `qitos.core.conversation` ExchangeLog contract (not yet a root export): ordered multimodal user/assistant/steering/tool-result items, provider-scoped call identities, raw/parsed argument status, immediate partial-result persistence in real completion order, declaration-ordered derived queries, recovery-safe synthetic closure and queued steering, defensive ownership boundaries, opaque continuation persistence with an explicitly non-private continuation-redacted diagnostic projection, strict `HistoryMessage` compatibility adapters, typed invariant errors, and versioned consumer-simulation fixtures. Engine/provider/checkpoint main paths and canonical C1-R `ToolResult` alignment remain pending.
