...
- Added a bounded, opt-in S3 live-provider qualification runner. It reads the three profile definitions from the internal matrix, separates credential references from configuration, enforces 12 requests/profile, 10,240 output tokens/request, a 180-second timeout, and zero automatic retries, counts only provider-native tool calls, requires external private evidence storage and a passing redacted sandbox attestation before configured live requests, and emits a sorted redacted JSON summary with typed nonzero failures.
