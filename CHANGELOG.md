...
- Added the v4 execution decomposition under `docs/v4/`: five actionable subtask designs — baseline absorption (campaign mechanism commits landing plan), conversation kernel (canonical message stack, reasoning/delivery policies, native parallel tool calls), native ACI toolset (grounded in the battle-tested READ/GREP/EDIT/BASH reference implementations), runtime context injection with evidence-grade memory, and the trajectory data plane with observability generalization.
- Added the v4 goal-and-metric definition (`docs/v4/00-goal-metric.md`): the 10%-code goal restated as two measurable acceptance metrics (mechanism-zero in the out-of-tree agent; a ≤10% reference agent with a pass-rate anchor), with the measured 50,872-LOC baseline breakdown of the campaign agent.
