...
- Accepted the A → C → B → D G1 final convergence after G1-R3 closed C-P3, requalified B's direct projection consumer without a runtime change, refreshed D's exact C producer receipt, and passed the Python 3.12.7 ratchet, lint/type, targeted, readiness, adversarial, and full-suite gates. S1 contract packages are authorized only from the final accepted baseline; no Task 02B, 03B–E, 05A, 12/13 runtime, provider-default, trajectory-v2, qita, or packaging behavior was started.
...
- Closed the G1 ToolResult JSON-admission, nested-ownership, and C-P3 projection gaps: tool calls reject recursive non-JSON values and non-finite numbers before interceptors, permissions, or execution; canonical/legacy construction and serialization detach nested caller-owned values; and sensitive model/trace mapping keys use deterministic collision-safe placeholders without exposing or hashing the raw key. Trace-safe omitted data is budgeted and aggregate/per-field loss facts account for redacted keys, omitted fields, and omitted characters.
