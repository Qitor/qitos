...
- Added the module-level `qitos.core.conversation` ExchangeLog contract (not yet a root export): ordered multimodal user/assistant/steering/tool-result items, provider-scoped call identities, raw/parsed argument status, declaration-ordered parallel batch closure with synthetic provenance, opaque continuation persistence with redacted safe projection, strict `HistoryMessage` compatibility adapters, typed invariant errors, and versioned Lane C/D semantic fixtures. Engine/provider/checkpoint main paths remain unchanged pending later v4 work packages.
