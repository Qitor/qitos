...
- Recorded the A → C → B → D G1 convergence candidate and its independently green ratchet, lint/type, targeted, full-suite, and readiness gates, then reopened G1 on C-P3 after adversarial probes showed sensitive mapping keys and trace-safe omitted keys can bypass ToolResult redaction/loss accounting. S1 remains blocked pending the bounded repair and combined requalification; no Task 02B, 03B–E, 05A, 12/13 runtime, provider-default, trajectory-v2, qita, or packaging behavior was started.
...
- Closed the G1 ToolResult JSON-admission and nested-ownership gaps: tool calls reject recursive non-JSON values and non-finite numbers before interceptors, permissions, or execution, and canonical/legacy construction and serialization detach nested caller-owned values. Scalar model/trace values use bounded redaction; sensitive mapping-key and omitted-key accounting remains the explicitly reopened C-P3 blocker.
